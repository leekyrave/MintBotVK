            ...
            switch ((int)$_GET['p']) {
                case 0: 
                    if((int)$_GET['s'] <= 25 and (int)$_GET['s'] > 0) {
                        $data = json_decode(file_get_contents('/home/admin/web/api.mint-plantation.ru/public_html/monitoring/arizonam_'.$_GET['s'].'.json'),true);
                        $result_array = array('error' => 0, 'project' => $_GET['p'], 'server' => $_GET['s'],array());
                        foreach ($data as $key => $value) {
                            foreach($value as $second_key => $second_value) {
                                if((int)$second_value['rank'] == 9) {
                        ...
                        if(count($result_array[0]) > 0) {
                            ...
                            $core->reportError($core::ERROR_NO_LEADERS,'No deputies');
                        }  
                    } else {
                ...
                case 1: 
                    if((int)$_GET['s'] <= 4 and (int)$_GET['s'] > 0) {
                        $data = json_decode(file_get_contents('/home/admin/web/api.mint-plantation.ru/public_html/monitoring/rodinam_'.$_GET['s'].'.json'),true);
                        $result_array = array('error' => 0, 'project' => $_GET['p'], 'server' => $_GET['s'], array());
                        foreach ($data as $key => $value) {
                            foreach($value as $second_key => $second_value) {
                                if((int)$second_value['rank'] == 9) {
